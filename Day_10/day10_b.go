...
func getScore(x,y int, from byte, topographicMap []string, reached map[struct{x,y int}]int){
	if x<0 || y<0 || y>=len(topographicMap) || x>=len(topographicMap[0])  || 
		topographicMap[y][x] != from+1{
		return
	}
	if topographicMap[y][x] == '9'{
		reached[struct{x,y int}{x,y}]++
		return
	}
